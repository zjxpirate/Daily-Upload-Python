...
# 5. compute the levenshtein distance
...
def levenshtein_distance(A, B):
    def compute_distance_between_prefixes(A_idx, B_idx):
        if A_idx < 0:
            ...
            # B is empty so so delete all of A's characters.
            return A_idx + 1
        ...
        if distance_between_prefixes[A_idx][B_idx] == -1:
            if A[A_idx] == B[B_idx]:
                ...
                print("min(substitute_last, add_last, delete_last): ", distance_between_prefixes[A_idx][B_idx])

        print("distance_between_prefixes: ", distance_between_prefixes)
